...
fn main() {
    ...
    // The native `zeg_run` FFI (`run`) links a static lib built from the C++
    // sources with a normal host C++ toolchain (not the RISC-V one). Only built
    // under the `ziskethone` feature, so a proving-only build (ELF only) needs no
    // C++ toolchain and no submodule.
    if std::env::var_os("CARGO_FEATURE_ZISKETHONE").is_some() {
        ensure_submodule_initialized();
        build_ffi();
    }
}
...
fn preflight_cxx_toolchain(cxx: &str) {
    ...
    // Unknown compiler (e.g. macOS AppleClang, MSVC): don't second-guess it.
}
